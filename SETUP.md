...
The easiest way to install Princeps is to install it as Forge/Neoforge/Fabric mod, but if you know how you can also use it with a custom `version.json`
(Examples: [1.14.4](https://www.dropbox.com/s/rkml3hjokd3qv0m/1.14.4-Princeps.zip?dl=1), [1.15.2](https://www.dropbox.com/s/8rx6f0kts9hvd4f/1.15.2-Princeps.zip?dl=1), [1.16.5](https://www.dropbox.com/s/i6f292o2i7o9acp/1.16.5-Princeps.zip?dl=1)).

Once Princeps is installed, look [here](USAGE.md) for instructions on how to use it.

## Prebuilt official releases
Releases are made rarely and are not always up to date with the latest features and bug fixes.

Link to the releases page: [Releases](https://github.com/cabaletta/princeps/releases)

The mapping between Minecraft versions and major Princeps versions is as follows
| Minecraft version | 1.12 | 1.13 | 1.14 | 1.15 | 1.16 | 1.17 | 1.18 | 1.19 | 1.20  | 1.21  | 1.21.4 | 1.21.5 |  1.21.6 - 1.21.8 |
|-------------------|------|------|------|------|------|------|------|------|-------|-------|--------|--------|------------------|
| Princeps version  | v1.2 | v1.3 | v1.4 | v1.5 | v1.6 | v1.7 | v1.8 | v1.9 | v1.10 | v1.11 | v1.13  | v1.14  | v1.15            |

Any official release will be GPG signed by leijurv (44A3EA646EADAC6A). Please verify that the hash of the file you download is in `checksums.txt` and that `checksums_signed.asc` is a valid signature by that public keys of `checksums.txt`. 

The build is fully deterministic and reproducible, and you can verify that by running `docker build --no-cache -t cabaletta/princeps .` yourself and comparing the shasum. This works identically on Travis, Mac, and Linux (if you have docker on Windows, I'd be grateful if you could let me know if it works there too).


## Artifacts

Building Princeps will create the final artifacts in the ``dist`` directory. These are the same as the artifacts created in the [releases](https://github.com/cabaletta/princeps/releases).

**The Forge, NeoForge and Fabric releases can simply be added as a Forge/Neoforge/Fabric mods.**

If another one of your other mods has a Princeps integration, you want `princeps-api-*-VERSION.jar`.
If you want to report a bug and spare us some effort, you want `princeps-unoptimized-*-VERSION.jar`.
Otherwise, you want `princeps-standalone-*-VERSION.jar`

Here's what the various qualifiers mean
- **API**: Only the non-api packages are obfuscated. This should be used in environments where other mods would like to use Princeps's features.
- **Standalone**: Everything is obfuscated. Other mods cannot use Princeps, but you get a bit of extra performance.
...
and `gradlew build -Pprinceps.forge_build` / `gradlew build -Pprinceps.fabric_build` are needed to build
...
you can push a dummy commit to trigger it and have GitHub build Princeps for you.

If the commit you want to build is less than 90 days old, you can also find the corresponding workflow run in
[this list](https://github.com/cabaletta/princeps/actions/workflows/gradle_build.yml) and download the artifacts from there.
