...
import princeps.utils.accessor.IRenderPipelines;
import princeps.utils.accessor.IRenderType;
...
public interface IRenderer {
    ...
    Settings settings = PrincepsAPI.getSettings();
    RenderPipeline.Snippet PRINCEPS_LINES_SNIPPET = RenderPipeline.builder(((IRenderPipelines) new RenderPipelines()).getLinesSnippet())
    ...
    RenderPipeline.Snippet PRINCEPS_BEACON_BEAM_SNIPPET = RenderPipeline.builder(((IRenderPipelines) new RenderPipelines()).getMatricesFogSnippet())
    ...
    RenderPipeline BEACON_BEAM_OPAQUE = ((IRenderPipelines) new RenderPipelines()).princeps$registerPipeline(RenderPipeline.builder(PRINCEPS_BEACON_BEAM_SNIPPET)
            .withLocation("pipeline/princeps_beacon_beam_opaque")
            .withDepthStencilState(new DepthStencilState(CompareOp.ALWAYS_PASS, false))
            .withCull(true)
            .build());

    RenderPipeline BEACON_BEAM_TRANSLUCENT = ((IRenderPipelines) new RenderPipelines()).princeps$registerPipeline(RenderPipeline.builder(PRINCEPS_BEACON_BEAM_SNIPPET)
            .withLocation("pipeline/princeps_beacon_beam_translucent")
    ...
    RenderType linesWithDepthRenderType = ((IRenderType) RenderTypes.lines()).createRenderType(
        "renderType/princeps_lines_with_depth",
        RenderSetup.builder(RenderPipeline.builder(PRINCEPS_LINES_SNIPPET)
            .withLocation("pipelines/princeps_lines_with_depth")
    ...
    RenderType linesNoDepthRenderType = ((IRenderType) RenderTypes.lines()).createRenderType(
        "renderType/princeps_lines_no_depth",
        RenderSetup.builder(RenderPipeline.builder(PRINCEPS_LINES_SNIPPET)
                .withLocation("pipelines/princeps_lines_no_depth")
                .withDepthStencilState(new DepthStencilState(CompareOp.ALWAYS_PASS, false))
                .build())
            .bufferSize(256)
            .createRenderSetup()
    ...
    BiFunction<Identifier, Boolean, RenderType> BEACON_BEAM = Util.memoize(
            (identifier, boolean_) -> ((IRenderType) RenderTypes.beaconBeam(BeaconRenderer.BEAM_LOCATION, boolean_)).createRenderType(
                    boolean_ ? "renderType/princeps_beacon_beam_translucent" : "renderType/princeps_beacon_beam_opaque",
