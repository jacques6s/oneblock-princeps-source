...
public class GuiClick extends Screen implements Helper {
    ...
    public void extractRenderState(GuiGraphicsExtractor stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (near != null && far != null) {
            ...
            LocalPlayer player = PrincepsAPI.getProvider().getPrimaryPrinceps().getPlayerContext().player();
        ...
        if (currentMouseOver != null) { //Catch this, or else a click into void will result in a crash
            if (event.button() == 0) {
                if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                    PrincepsAPI.getProvider().getPrimaryPrinceps().getSelectionManager().removeAllSelections();
                    PrincepsAPI.getProvider().getPrimaryPrinceps().getSelectionManager().addSelection(BetterBlockPos.from(clickStart), BetterBlockPos.from(currentMouseOver));
                    MutableComponent component = Component.literal("Selection made! For usage: " + Princeps.settings().prefix.value + "help sel");
                    ...
                    PrincepsAPI.getProvider().getPrimaryPrinceps().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver));
                }
            } else if (event.button() == 1) {
                PrincepsAPI.getProvider().getPrimaryPrinceps().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver.above()));
            }
        }
        clickStart = null;
    ...
    public void onRender(PoseStack modelViewStack, Matrix4f projectionMatrix) {
        ...
        if (currentMouseOver != null) {
            ...
            if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                ...
                IRenderer.emitAABB(bufferBuilder, modelViewStack, new AABB(Math.min(a.x, b.x), Math.min(a.y, b.y), Math.min(a.z, b.z), Math.max(a.x, b.x) + 1, Math.max(a.y, b.y) + 1, Math.max(a.z, b.z) + 1), Princeps.settings().pathRenderLineWidthPixels.value);
