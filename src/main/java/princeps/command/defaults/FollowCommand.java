...
import princeps.KeepName;
import princeps.api.IPrinceps;
import princeps.api.command.Command;
import princeps.api.command.argument.IArgConsumer;
import princeps.api.command.datatypes.EntityClassById;
import princeps.api.command.datatypes.IDatatypeFor;
import princeps.api.command.datatypes.NearbyPlayer;
import princeps.api.command.exception.CommandErrorMessageException;
...
public class FollowCommand extends Command {
    ...
    public FollowCommand(IPrinceps princeps) {
        super(princeps, "follow");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (args.hasExactlyOne()) {
            princeps.getFollowProcess().follow((group = args.getEnum(FollowGroup.class)).filter);
        } else {
            args.requireMin(2);
            group = null;
            ...
            princeps.getFollowProcess().follow(
                    classes.isEmpty()
                            ? entities::contains
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The follow command tells Princeps to follow certain kinds of entities.",
                "",
                "Usage:",
