...
import princeps.api.command.datatypes.ForWaypoints;
...
public class WaypointsCommand extends Command {
    ...
    public WaypointsCommand(IPrinceps princeps) {
        super(princeps, "waypoints", "waypoint", "wp");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.LIST) {
            ...
            IWaypoint[] waypoints = tag != null
                    ? ForWaypoints.getWaypointsByTag(this.princeps, tag)
                    : ForWaypoints.getWaypoints(this.princeps);
            ...
            ForWaypoints.waypoints(this.princeps).addWaypoint(waypoint);
            ...
            IWaypoint[] waypoints = ForWaypoints.getWaypointsByTag(this.princeps, tag);
            for (IWaypoint waypoint : waypoints) {
                ForWaypoints.waypoints(this.princeps).removeWaypoint(waypoint);
            }
            deletedWaypoints.computeIfAbsent(princeps.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).addAll(Arrays.<IWaypoint>asList(waypoints));
            ...
            List<IWaypoint> deletedWaypoints = this.deletedWaypoints.getOrDefault(princeps.getWorldProvider().getCurrentWorld(), Collections.emptyList());
            ...
            waypoints.forEach(ForWaypoints.waypoints(this.princeps)::addWaypoint);
            ...
            if (waypoint == null) {
                ...
                if (action == Action.INFO) {
                    ...
                    recreateComponent.setStyle(recreateComponent.getStyle().withClickEvent(new ClickEvent.SuggestCommand(
                            String.format(
                                    "%s%s save %s %s %s %s %s",
                                    Princeps.settings().prefix.value, // This uses the normal prefix because it is run by the user.
                                    label,
                                    waypoint.getTag().getName(),
                                    waypoint.getName(),
                    ...
                    ForWaypoints.waypoints(this.princeps).removeWaypoint(waypoint);
                    deletedWaypoints.computeIfAbsent(princeps.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).add(waypoint);
                    ...
                    princeps.getCustomGoalProcess().setGoal(goal);
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The waypoint command allows you to manage Princeps's waypoints.",
                "",
