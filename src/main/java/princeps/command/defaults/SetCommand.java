...
import static princeps.api.utils.SettingsUtil.*;

public class SetCommand extends Command {

    public SetCommand(IPrinceps princeps) {
        super(princeps, "set", "setting", "settings");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (Arrays.asList("s", "save").contains(arg)) {
            SettingsUtil.save(Princeps.settings());
        ...
        if (Arrays.asList("load", "ld").contains(arg)) {
            ...
            // reset to defaults
            SettingsUtil.modifiedSettings(Princeps.settings()).forEach(Settings.Setting::reset);
            // then load from disk
            SettingsUtil.readAndApply(Princeps.settings(), file);
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate =
                    (viewModified ? SettingsUtil.modifiedSettings(Princeps.settings()) : Princeps.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    setting -> {
                        ...
                        String commandSuggestion = Princeps.settings().prefix.value + String.format("set %s ", setting.getName());
        ...
        Settings.Setting<?> setting = Princeps.settings().allSettings.stream()
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                try {
                    SettingsUtil.parseAndApply(Princeps.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value && !Princeps.settings().chatControlAnyway.value) ||
                    setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value && !Princeps.settings().chatControl.value) {
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, Minecraft.getInstance().gameDirectory.toPath().resolve("princeps").toFile());
                }
                Settings.Setting setting = Princeps.settings().byLowerName.get(arg.toLowerCase(Locale.US));
                if (setting != null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using the set command, you can manage all of Princeps's settings. Almost every aspect is controlled by these settings - go wild!",
                "",
                "Usage:",
                ...
                "> set load [filename] - Load settings from another file in your minecraft/princeps"
        );
    }
}
