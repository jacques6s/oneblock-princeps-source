...
import princeps.api.command.datatypes.ForAxis;
import princeps.api.command.datatypes.ForBlockOptionalMeta;
import princeps.api.command.datatypes.ForDirection;
...
import princeps.api.schematic.mask.shape.CylinderMask;
import princeps.api.schematic.mask.shape.SphereMask;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = princeps.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(IPrinceps princeps) {
        super(princeps, "sel", "selection", "s");
        princeps.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!Princeps.settings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = Princeps.settings().colorSelectionPos1.value;
                float opacity = Princeps.settings().selectionOpacity.value;
                float lineWidth = Princeps.settings().selectionLineWidth.value;
                boolean ignoreDepth = Princeps.settings().renderSelectionIgnoreDepth.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            princeps.getBuilderProcess().build("Fill", composite, origin);
            ...
            princeps.getBuilderProcess().build("Fill", clipboard, pos.offset(clipboardOffset));
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate Princeps's selections, similarly to WorldEdit.",
                "",
