...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(Princeps princeps) {
        super(princeps);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            princeps.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            princeps.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        princeps.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    ...
    private void tickPath() {
        ...
        synchronized (pathPlanLock) {
            ...
            if (Princeps.settings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < Princeps.settings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(princeps, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !princeps.getElytraProcess().isActive() || princeps.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        princeps.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = Princeps.settings().primaryTimeoutMS.value;
            failureTimeout = Princeps.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = Princeps.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = Princeps.settings().planAheadFailureTimeoutMS.value;
        }
    ...
    private AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (Princeps.settings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
            BlockPos pos = ((IGoalRenderPos) goal).getGoalPos();
            if (!context.bsi.worldContainsLoadedChunk(pos.getX(), pos.getZ())) {
                transformed = new GoalXZ(pos.getX(), pos.getZ());
            }
        }
        Favoring favoring = new Favoring(context.getPrinceps().getPlayerContext(), previous, context);
        BetterBlockPos feet = ctx.playerFeet();
