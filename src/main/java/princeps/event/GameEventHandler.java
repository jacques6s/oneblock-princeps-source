...
import princeps.api.event.listener.IGameEventListener;
...
public final class GameEventHandler implements IEventBus, Helper {
    ...
    public GameEventHandler(Princeps princeps) {
    ...
    public final void onTick(TickEvent event) {
        if (event.getType() == TickEvent.Type.IN) {
            try {
                princeps.bsi = new BlockStateInterface(princeps.getPlayerContext(), true);
            } catch (Exception ex) {
                ex.printStackTrace();
                princeps.bsi = null;
            }
    ...
    public void onChunkEvent(ChunkEvent event) {
        ...
        if (event.isPostPopulate() || isPreUnload) {
            princeps.getWorldProvider().ifWorldLoaded(worldData -> {
    ...
    public void onBlockChange(BlockChangeEvent event) {
        if (Princeps.settings().repackOnAnyBlockChange.value) {
            ...
            if (keepingTrackOf) {
                princeps.getWorldProvider().ifWorldLoaded(worldData -> {
                    final Level world = princeps.getPlayerContext().world();
    ...
    public final void onWorldEvent(WorldEvent event) {
        WorldProvider cache = princeps.getWorldProvider();
