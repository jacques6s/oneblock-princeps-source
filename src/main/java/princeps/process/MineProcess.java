...
import princeps.api.process.IMineProcess;
 ...
 * @author leijurv
 */
public final class MineProcess extends PrincepsProcessHelper implements IMineProcess {
    ...
    public MineProcess(Princeps princeps) {
        super(princeps);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && Princeps.settings().blacklistClosestOnFailure.value) {
                logDirect("Unable to find any path to " + filter + ", blacklisting presumably unreachable closest instance...");
                if (Princeps.settings().notificationOnMineFail.value) {
        ...
        updateLoucaSystem();
        int mineGoalUpdateInterval = Princeps.settings().mineGoalUpdateInterval.value;
        List<BlockPos> curr = new ArrayList<>(knownOreLocations);
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(princeps, true);
            Princeps.getExecutor().execute(() -> rescan(curr, context));
        }
        if (Princeps.settings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().onGround()) {
            ...
            BlockState state = princeps.bsi.get0(pos);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + Princeps.settings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = Princeps.settings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(princeps);
            List<BlockPos> locs2 = prune(context, new ArrayList<>(locs), filter, Princeps.settings().mineMaxOreLocationsCount.value, blacklist, droppedItemsScan());
        ...
        if (!legit && !Princeps.settings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = Princeps.settings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!princeps.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        List<BlockPos> locs = searchWorld(context, filter, Princeps.settings().mineMaxOreLocationsCount.value, already, blacklist, dropped);
        locs.addAll(dropped);
        if (locs.isEmpty() && !Princeps.settings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (Princeps.settings().internalMiningAirException.value && state.getBlock() instanceof AirBlock) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        boolean assumeVerticalShaftMine = !(princeps.bsi.get0(loc.above()).getBlock() instanceof FallingBlock);
        if (!Princeps.settings().forceInternalMining.value) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!Princeps.settings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.princeps.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        Princeps.settings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (Princeps.settings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(PrincepsAPI.getProvider().getWorldScanner().scanChunkRadius(
                    ctx.getPrinceps().getPlayerContext(),
                    filter,
                    max,
                    10,
                    32
    ...
    private boolean addNearby() {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((Princeps.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distSqr(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx, pos, fakedBlockReachDistance).isPresent()) {
        ...
        knownOreLocations = prune(new CalculationContext(princeps), knownOreLocations, filter, Princeps.settings().mineMaxOreLocationsCount.value, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (Princeps.settings().allowOnlyExposedOres.value) {
                ...
                .filter(pos -> pos.getY() >= Princeps.settings().minYLevelWhileMining.value + ctx.world.dimensionType().minY())

                .filter(pos -> pos.getY() <= Princeps.settings().maxYLevelWhileMining.value)

                .filter(pos -> !blacklist.contains(pos))

                .sorted(Comparator.comparingDouble(ctx.getPrinceps().getPlayerContext().player().blockPosition()::distSqr))
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = Princeps.settings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(princeps));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!Princeps.settings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> Princeps.settings().allowBreakAnyway.value.contains(e.getBlock()))
