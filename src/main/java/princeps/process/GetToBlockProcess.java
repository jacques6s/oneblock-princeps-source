...
import princeps.api.utils.BlockOptionalMetaLookup;
...
public final class GetToBlockProcess extends PrincepsProcessHelper implements IGetToBlockProcess {
    ...
    public GetToBlockProcess(Princeps princeps) {
        super(princeps);
    }
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (knownLocations.isEmpty()) {
            if (Princeps.settings().exploreForBlocks.value && !calcFailed) {
        ...
        if (calcFailed) {
            if (Princeps.settings().blacklistClosestOnFailure.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            ...
            Princeps.getExecutor().execute(() -> rescan(current, context));
        }
        if (goal.isInGoal(ctx.playerFeet()) && goal.isInGoal(princeps.getPathingBehavior().pathStart()) && isSafeToCancel) {
            // we're there
    ...
    public class GetToBlockCalculationContext extends CalculationContext {
        ...
        public GetToBlockCalculationContext(boolean forUseOnAnotherThread) {
            super(GetToBlockProcess.super.princeps, forUseOnAnotherThread);
        }
    ...
    private Goal createGoal(BlockPos pos) {
        ...
        if (blockOnTopMustBeRemoved(gettingTo.getBlock()) && MovementHelper.isBlockNormalCube(princeps.bsi.get0(pos.above()))) { // TODO this should be the check for chest openability
    ...
    private boolean rightClick() {
        for (BlockPos pos : knownLocations) {
            ...
            if (reachable.isPresent()) {
                princeps.getLookBehavior().updateTarget(reachable.get(), true);
                if (knownLocations.contains(ctx.getSelectedBlock().orElse(null))) {
                    princeps.getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, true); // TODO find some way to right click even if we're in an ESC menu
    ...
    private boolean walkIntoInsteadOfAdjacent(Block block) {
        if (!Princeps.settings().enterPortal.value) {
            return false;
        }
    ...
    private boolean rightClickOnArrival(Block block) {
        if (!Princeps.settings().rightClickContainerOnArrival.value) {
            return false;
        }
