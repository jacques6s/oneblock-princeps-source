...
import static princeps.utils.PrincepsMath.fastCeil;
import static princeps.utils.PrincepsMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(Princeps princeps, ElytraProcess process, BlockPos destination, boolean appendDestination) {
        ...
        this.context = NetherPathfinderContext.forLevel(Princeps.settings().elytraNetherSeed.value, this.ctx.world());
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (!this.blockedLines.isEmpty() && Princeps.settings().elytraRenderRaytraces.value) {
        ...
        if (this.simulationLine != null && Princeps.settings().elytraRenderSimulation.value) {
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof ClientboundPlayerPositionPacket) {
            ctx.minecraft().execute(() -> {
                this.remainingSetBackTicks = Princeps.settings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!Princeps.settings().elytraAutoJump.value || ctx.player().isFallFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Princeps.settings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.player().chunkPosition().x(), ctx.player().chunkPosition().z(), Princeps.settings().elytraCacheCullDistance.value, this.boi);
    ...
    /**
     * Called by {@link princeps.process.ElytraProcess#onTick(boolean, boolean)} when the process is in control and the player is flying
     */
    public void tick() {
        ...
        princeps.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and more (if we're in a corner or near some blocks, it will have to relax its constraints a bit)
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = Princeps.settings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !Princeps.settings().elytraConserveFireworks.value || ctx.player().position().y < goingTo.y + 5;
        ...
        final double elytraFireworkSpeed = Princeps.settings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
            ...
            if (!princeps.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !princeps.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.princeps.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3 dest, final Double growAmount) {
        ...
        if (Princeps.settings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3 start, Vec3 dest, boolean ignoreLava) {
        ...
        if (Princeps.settings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90 : Math.max(goodPitch - Princeps.settings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + Princeps.settings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3 goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3 : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks()) : Princeps.settings().elytraSimulationTicks.value;
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = Princeps.settings().ticksBetweenInventoryMoves.value;
            }
        }
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getMaxDamage() - slot.getDamageValue()) > Princeps.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!Princeps.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }
    ...
    void logVerbose(String message) {
        if (Princeps.settings().elytraChatSpam.value) {
            logDebug(message);
        }
    }
}
