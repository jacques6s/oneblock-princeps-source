...
public final class BackfillProcess extends PrincepsProcessHelper {

    public HashMap<BlockPos, BlockState> blocksToReplace = new HashMap<>();

    public BackfillProcess(Princeps princeps) {
        super(princeps);
    }
    ...
    public boolean isActive() {
        ...
        if (!Princeps.settings().backfill.value) {
            return false;
        }
        if (Princeps.settings().allowParkour.value) {
            logDirect("Backfill cannot be used with allowParkour true");
            Princeps.settings().backfill.value = false;
            return false;
        }
    ...
    private void amIBreakingABlockHMMMMMMM() {
        if (!ctx.getSelectedBlock().isPresent() || !princeps.getPathingBehavior().isPathing()) {
            return;
        }
    ...
    public List<BlockPos> toFillIn() {
        return blocksToReplace
                ...
                .filter(pos -> princeps.getBuilderProcess().placementPlausible(pos, Blocks.DIRT.defaultBlockState()))
