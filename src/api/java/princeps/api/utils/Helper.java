 ...
 * chat and console logging mechanisms, and the Princeps chat prefix.
 *
 * @author Brady
 * @since 8/1/2018
 */
public interface Helper {
     ...
     */
    GuiMessageTag MESSAGE_TAG = new GuiMessageTag(0xFF55FF, null, Component.literal("Princeps message."), "Princeps");
    ...
    default void logToast(Component title, Component message) {
        Minecraft.getInstance().execute(() -> PrincepsAPI.getSettings().toaster.value.accept(title, message));
    }
    ...
    default void logNotification(String message, boolean error) {
        if (PrincepsAPI.getSettings().desktopNotifications.value) {
    ...
    default void logNotificationDirect(String message, boolean error) {
        Minecraft.getInstance().execute(() -> PrincepsAPI.getSettings().notifier.value.accept(message, error));
    }
    ...
    /**
     * Send components to chat with the [Princeps] prefix
     *
    ...
    default void logDirect(boolean logAsToast, Component... components) {
        ...
        if (!logAsToast && !PrincepsAPI.getSettings().useMessageTag.value) {
        ...
        if (logAsToast) {
            ...
            Minecraft.getInstance().execute(() -> PrincepsAPI.getSettings().logger.value.accept(component));
        }
    }
    ...
    default void logDirect(Component... components) {
        logDirect(PrincepsAPI.getSettings().logAsToast.value, components);
    }
    ...
    default void logDirect(String message, ChatFormatting color) {
        logDirect(message, color, PrincepsAPI.getSettings().logAsToast.value);
    }
    ...
    default void logDirect(String message) {
        logDirect(message, PrincepsAPI.getSettings().logAsToast.value);
    }

    default void logUnhandledException(final Throwable exception) {
        HELPER.logDirect("An unhandled exception occurred. " +
                        "The error is in your game's log, please report this at https://github.com/cabaletta/princeps/issues",
                ChatFormatting.RED);
