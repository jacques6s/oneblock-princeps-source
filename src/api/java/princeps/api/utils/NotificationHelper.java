...
public class NotificationHelper {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Princeps");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Princeps");
                    tray.add(trayIcon);
                }

                trayIcon.displayMessage("Princeps", text, error ? TrayIcon.MessageType.ERROR : TrayIcon.MessageType.INFO);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private static void mac(String text) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + text + "\" with title \"Princeps\"");
        try {
    ...
    private static void linux(String text) {
        ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Princeps", text);
        try {
            processBuilder.start();
        } catch (IOException e) {
            e.printStackTrace();
